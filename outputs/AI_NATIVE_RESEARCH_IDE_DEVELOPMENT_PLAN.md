...
- Clerk 로그인, 검증된 Clerk user ID로 격리하는 Neon 개인 LaTeX 프로젝트 수동 스냅샷
...
- 선택형 Clerk 로그인과 사용자 소유 Neon 프로젝트 수동 스냅샷
...
Yjs/WebRTC를 이용한 링크 기반 공동 편집은 MVP에 포함한다. Clerk 로그인과 Neon의 최소 `projects` schema를 통한 개인 스냅샷은 선택형으로 제공한다. 모든 쿼리는 서버에서 검증한 Clerk user ID로 제한한다. Clerk Organizations, audit log와 collaboration server 영속화는 핵심 편집 경험이 검증된 뒤 추가한다.
